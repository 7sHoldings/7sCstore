...
import { setCardForDay, resetCardForDay } from "./actions";
...
export default function EditCardButton({
  ...
  const [state, action, pending] = useActionState(setCardForDay, {});
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} className="ft-btn-ghost p-1.5 rounded-md" aria-label="Edit credit/debit" title="Edit credit/debit">
  ...
  return (
    <Modal title={`Credit / Debit · ${label}`} onClose={() => setOpen(false)}>
      <form action={action} className="space-y-4">
        <input type="hidden" name="date" value={date} />
        <div className="grid grid-cols-2 gap-3 rounded-lg bg-surface-container-low p-3">
          <Stat label="POS Card Batch" value={pos != null ? fmtMoney(pos) : "—"} />
        ...
        <div>
          <label className="ft-label" htmlFor="card-amt">Credit / Debit amount</label>
          <input id="card-amt" name="amount" type="number" step="0.01" min="0" className="ft-input" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" required autoFocus />
          <p className="text-[11px] text-on-surface-variant mt-1">Saved as a manual override — it wins over the POS card batch and survives the next sync.</p>
        </div>
        ...
        <div className="flex gap-2 pt-1">
          {manual != null && pos != null && (
            <button
              ...
              onClick={() => startReset(async () => { await resetCardForDay(date); setOpen(false); })}
              className="ft-btn-ghost justify-center"
              title="Discard the manual override and use the POS card batch"
            >
